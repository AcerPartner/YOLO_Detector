...
void Inference::detect(const cv::Mat& CVInputImage, const vector<uint8_t>& modelData, vector<YOLO_output>* ret)
{
    ...
    Ort::ShapeInferContext::Ints output_shape = tensor_info.GetShape();
    if (output_shape.size() != 2) return;
    size_t output0 = output_shape[0], output1 = output_shape[1];
    if (output1 != 7) // batch_id, x0, y0, x1, y1, cls_id, score
        return;
    float* output = output_tensor->GetTensorMutableData<float>();
    for (size_t i = 0; i < output0; ++i)
    {
        YOLO_output out;
        out.batch_id = output[i * output1 + 0];
        out.x0 = output[i * output1 + 1];
        out.y0 = output[i * output1 + 2];
        out.x1 = output[i * output1 + 3];
        out.y1 = output[i * output1 + 4];
        out.cls_id = output[i * output1 + 5];
        out.score = output[i * output1 + 6];
        ret->push_back(out);
    }
}